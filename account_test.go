...
func TestMakeRequestWithBody(t *testing.T) {
	server, api := startMockServer(t, []RequestHandler{{
		...
		EstimatedRequestHeaders: map[string]string{"Content-Type": "application/json"},
		EstimatedRequestContent: `{"field1":"value1","field2":"value with space"}`,
		ResponseContent:         `{"test": "test"}`}})
	defer server.Close()
